...
sql = """
SELECT table_name,
       IFF(UPPER(view_definition) LIKE '%BASE_CW_DP_TRT_V_CS_BILLING_PRODUCT_USAGE%', 'Y', 'N') AS references_raw_usage,
       IFF(UPPER(view_definition) LIKE '%THIRD_PARTY_RECON_SOURCE_TRT_PROD%', 'Y', 'N') AS references_source_trt_prod
