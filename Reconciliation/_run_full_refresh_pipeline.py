...
Data-bearing ingestion and invoice parsing run by default. Code-only smart-skip is
available as an explicit optimization, but is never treated as a freshness proof.
"""
...
def verify_authoritative_freshness(run_started_at: datetime) -> bool:
    """Require every canonical data-bearing table to be touched by this run."""
    print("\n=== Authoritative freshness gate ===", flush=True)
    conn = get_snowflake_connection(
        role="DEVELOPER",
        warehouse="REPORTING_WH",
        database="ANALYTICS_DEV",
    ...
    try:
        placeholders = ", ".join(["%s"] * len(AUTHORITATIVE_TABLES))
        with conn.cursor() as cur:
            cur.execute(
                f"""
            ...
            altered_by_table = {str(name): altered for name, altered in cur.fetchall()}

        failures: list[str] = []
        for table_name in AUTHORITATIVE_TABLES:
            last_altered = altered_by_table.get(table_name)
            comparable_last_altered = last_altered
            if last_altered is not None and last_altered.tzinfo is None:
                comparable_last_altered = last_altered.replace(tzinfo=timezone.utc)
            is_fresh = (
                comparable_last_altered is not None
                and comparable_last_altered >= run_started_at
            )
            status = "PASS" if is_fresh else "FAIL"
            print(f"  {status}  {table_name:<55} {last_altered}", flush=True)
            if not is_fresh:
                failures.append(table_name)
        if failures:
            print(
                "FAILED: canonical table(s) were not refreshed after this run began: "
                + ", ".join(failures),
                flush=True,
            )
            return False
        print("Authoritative freshness gate: PASS", flush=True)
        return True
    except Exception as exc:
        print(f"FAILED authoritative freshness check: {exc}", flush=True)
        return False
    finally:
        conn.close()
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--enable-smart-skip",
        action="store_true",
        help="Opt in to code-signature-only skipping. This does not prove source-data freshness.",
    )
    p.add_argument(
        "--disable-smart-skip",
        action="store_true",
...
def main() -> int:
    ...
    smart_skip_enabled = (
        args.enable_smart_skip
        and not args.disable_smart_skip
        and not full_refresh_now
    )
    ...
    if skip_ingestion_unchanged:
        ...
        print(
            "Skipped (explicit code-only smart-skip): ingestion scripts unchanged; "
            "source-data freshness was not evaluated.",
            flush=True,
        )
    ...
    if skip_invoices_unchanged:
        ...
        print(
            "Skipped (explicit code-only smart-skip): invoice parser unchanged for "
            "the selected --from-month; source-data freshness was not evaluated.",
            flush=True,
        )
    ...
    if skipped_steps:
        state["last_run_status"] = "partial_or_smart_skipped"
        state["last_run_completed_at_utc"] = datetime.now(timezone.utc).isoformat()
        _save_state(state)
        print(
            "\nPipeline run complete, but it was not recorded as an authoritative full "
            "refresh because one or more steps were skipped.",
            flush=True,
        )
        return 0

    if not verify_authoritative_freshness(run_started_at):
        state["last_run_status"] = "failed_freshness_gate"
        _save_state(state)
        return 1

    state["recon_status"] = "ok"
    state["last_run_status"] = "authoritative_full_refresh_ok"
