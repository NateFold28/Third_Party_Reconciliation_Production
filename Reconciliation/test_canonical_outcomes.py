...
class StrictOutcomeCaseTests(unittest.TestCase):
    ...
    def test_recon_team_full_output_export_is_unfiltered_and_complete(self) -> None:
        ...
        self.assertNotIn('st.caption("Display grain: Vendor × Invoice × SKU")', app)
        self.assertNotIn('st.caption(f"Comparison grain:', app)
        self.assertNotIn("Unallocated {usage_source_label.lower()} rows", app)
    ...
    def test_invoice_usage_control_has_vendor_specific_alignment(self) -> None:
        ...
        self.assertIn("webroot_invoice_stream_registry AS", intra)
        self.assertIn("WHEN UPPER(i.vendor) = 'WEBROOT' THEN i.invoice_id", intra)
